...
class CodeGenerator:
    ...
    def visit_BinOp(self, node: BinOp):
        ...
        if node.op.type == PLUS:
            ...
            self.result.append("cmp %rbx, %rax")
            self.result.append("je .positive_floor_divide")
            self.result.append("cmpq $0, %rbx")
            self.result.append("je .zero_floor_divide")
            self.result.append("movq $-1, %rax")
            self.result.append(".negative_floor_divide:")
            self.result.append("addq %rbx, %rax")
            self.result.append("cmp %rax, %rbx")
            self.result.append("jge .negative_floor_divide")
            self.result.append("jmp .floor_divide_done")
            self.result.append(".positive_floor_divide:")
            self.result.append("movq %rax, %rbx")
            self.result.append(".floor_divide_done:")
